...
function LikeButton({postTitle}) {
  ...
  const [likecount, setIsLikeCount] = useState(0); // 좋아요 카운트 상태

  // 좋아요 버튼 클릭 이벤트 핸들러(담당자) : 좋아요 클릭하면 실행될 함수 객체
  const handleLikeClick = () => {
    setIsLiked(!isLiked); // 좋아요 상태 토글 true -> false, false -> true
    if (isLiked) {
      setIsLikeCount(likecount - 1); // 좋아요 취소시 - 1
    } else {
      setIsLikeCount(likecount + 1); // 좋아요시 + 1
    }
  };
  ...
  return (
    <div className="like-button">
      ...
      {/* 좋아요 개수 표시 */}
      <p className="like-count">{postTitle} 좋아요 {likecount}개</p>
      {/* 리셋 버튼 */}
