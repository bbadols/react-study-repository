...
const PostDetailPage = () => {
  ...
  // 컴포넌트 마운트 시 게시글 조회 and 게시물 번호가 업데이트시 실행
  useEffect(() => {
    // 게시물 조회 함수
    const fetchPost = async () => {
      try {
        setLoading(true);
        setError("");
        // API Server에서 게시글을 조회한다 (인증이 필요하므로 jwt token을 request header에 저장해 함께 전송
        // -> axios interceptor가 대행)
    ...
    fetchPost();
  }, [id]); // 의존성 배열 정보가 변경될 때 리렌더링됨(즉 게시물 번호가 변경되면 상세 게시물 정보를 업데이트)
